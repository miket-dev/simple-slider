class MouseHandler {
  ...
  fixPosition(position) {
    if (this.steps) {
      let step = this.steps
        ...
        .reduce((a, b) => {
          return Math.abs(b - (position.x - this.parentMin)) <
            Math.abs(a - (position.x - this.parentMin))
            ? b
            : a;
        });
  ...
  currentValue() {
    ...
    return (
      this.currentPercent() * (this.maxValue - this.minValue) + this.minValue
    );
  }

  setPosition(x) {
    this.lastKnownPosition =
      x * ((this.parentMax - this.parentMin) / 100) + this.parentMin;
